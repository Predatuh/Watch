...
/** Phone transport — same Firebase backend as the watch app. */
object FirebaseNoteTransport : NoteTransport {
    ...
    override suspend fun initialize() {
        ...
        if (user != null && !user.isAnonymous) onAuthenticated(user.uid) else statusText = "Sign in to start"
        initialized = true
    }

    override suspend fun signUp(email: String, password: String): String? = try {
        auth.createUserWithEmailAndPassword(email.trim(), password).await().user?.uid?.let { onAuthenticated(it) }
        null
    } catch (e: Exception) { authError(e) }

    override suspend fun signIn(email: String, password: String): String? = try {
        auth.signInWithEmailAndPassword(email.trim(), password).await().user?.uid?.let { onAuthenticated(it) }
        null
    } catch (e: Exception) { authError(e) }

    override suspend fun signOut() {
        inboxListener?.remove(); friendsListener?.remove()
        auth.signOut()
        uid = null; isSignedIn = false; myUsername = null; friends = emptyList()
    ...
    private suspend fun onAuthenticated(id: String) {
        ...
        try {
            ...
            startInboxListener(id); startFriendsListener(id); registerPushToken(id)
    ...
    private suspend fun registerPushToken(id: String) {
        runCatching {
            ...
            db.collection("users").document(id).set(mapOf("fcmToken" to token), SetOptions.merge()).await()
        }
    }
        ...
        return try {
            val claimed = db.runTransaction { tx ->
                ...
                if (snap.exists() && snap.getString("uid") != me) {
                    ...
                    tx.set(db.collection("users").document(me), mapOf("username" to wanted), SetOptions.merge())
                    true
                }
            }.await()
            if (claimed != true) return UsernameResult.TAKEN
            val old = myUsername
            if (old != null && old != wanted) runCatching { db.collection("usernames").document(old).delete().await() }
            myUsername = wanted
    ...
    override suspend fun addFriend(username: String): Friend? {
        ...
        if (name.isEmpty()) { statusText = "Enter a username"; return null }
        return try {
            val theirUid = db.collection("usernames").document(name).get().await().getString("uid")
                ?: run { statusText = "No user @$name"; return null }
    ...
    private fun authError(e: Exception): String {
        ...
        return when {
            ...
            m.contains("already", true) || m.contains("EMAIL_EXISTS", true) -> "Email already in use"
            ...
            m.contains("not allowed", true) -> "Email sign-in isn't enabled yet"
    ...
    private fun payloadToMap(payload: NotePayload): Map<String, Any> = when (payload) {
        is NotePayload.ExpressionNote -> mapOf("kind" to "expression", "expressionId" to payload.expressionId)
        is NotePayload.FireworkNote -> mapOf("kind" to "firework", "fireworkType" to payload.type.name)
    ...
    private fun mapToPayload(data: Map<String, Any?>): NotePayload? = when (data["kind"]) {
        ...
        "drawing" -> {
            ...
            val strokes = rawStrokes.mapNotNull { raw ->
                ...
                var i = 0
                while (i + 1 < flat.size) { points.add(Offset(flat[i], flat[i + 1])); i += 2 }
